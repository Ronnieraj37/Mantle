...
import privacyCheck from "../assets/animation_lk02awun.json";
import phoneAnimation from "../assets/animation_lk04hr9h.json";

const Registration = ({ contract, setisVerified, setnumber }) => {
  ...
  const onSignUp = async () => {
    const res = await contract.checkIfNum(num);
    if (res === false) {
      toast.error("Number Already Linked !",
        {
          style: {
            borderRadius: '10px',
            background: '#333',
  ...
  return (
    <div className='flex flex-col items-center w-[1000px] mt-16'>
      ...
      {!verification && <div className='flex flex-col items-center justify-center rounded-xl'>
        ...
        <Lottie animationData={privacyCheck} loop={false} className=" scale-[220%] mt-16 h-[90px]" />
      ...
      {verification &&
        <div className='flex items-center flex-col'>
          ...
          <Lottie animationData={phoneAnimation} className=" scale-[160%] mb-4 h-[90px]" />
