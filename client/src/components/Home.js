...
import PeerChat from "../artifacts/contracts/facets/Peerchat.sol/Peerchat.json";
  ...
  const connectFetch = async () => {
    const loadProvider = async (provider) => {
      if (provider) {
        ...
        try {
          ...
          if (switchError.code === 4902) {
            ...
            window.ethereum
              ...
              .catch((error) => {
                toast.error("Add Amoy to MetaMask", {
                  style: {
                    borderRadius: "10px",
                    background: "#333",
                    color: "#fff",
        ...
        let contractAddress = "0xE6548fDD1a8eEe78012d0E3e8A1e0ACb7788AC40"; //diamond amoy
    ...
    <div className="flex relative w-[1300px]">
      <div className="flex font-serif  ">
        <div className=" flex flex-row mt-4 mb-[30px]">
          <h1 className="text-blue-500 px-2  rounded-3xl z-0 text-4xl">
            Vendork
          </h1>
        </div>
        <div className="absolute top-5 right-5 items-end justify-end">
          {connected ? (
            <div className=" py-1 px-4 rounded-xl bg-green-600">Connected</div>
          ) : (
            <button
              className=" py-1  px-4 rounded-xl bg-blue-500"
              ...
              <div className="mt-36 flex flex-col items-center">
                <div className="flex p-3 items-center justify-around ">
                  <input
                    ...
                    className="m-1 text-black px-1 py-2 rounded-xl"
                    required
                    placeholder=" Name"
                    type="text"
                  />
                  <input
                    ...
                    className="m-1 text-black px-1 py-2 disabled:bg-slate-300 rounded-xl"
                    required
                    placeholder="999.."
                    type="number"
                  />
                ...
                <input
                  ...
                  className="m-3 px-1 py-2 w-[380px] h-[60px]  text-black rounded-xl"
                  placeholder="About"
                  defaultValue="Hey I'm using Vendork"
                  type="text"
                />
                <button
                  className="flex items-center py-1 px-4  rounded-xl bg-blue-500"
                  onClick={addUser}
                >
                  Create Account
                </button>
